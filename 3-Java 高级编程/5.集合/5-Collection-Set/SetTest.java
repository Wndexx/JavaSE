...
/*
    Set 接口的框架：
         ...
         |---- Collection 接口：单列集合，用来存储一个一个的对象
            ...
            |---- Set 接口：存储无序的、不可重复的数据    ---> 高中讲的 "集合"：无序性，确定性，互异性
                ...
                |---- HashSet：作为 Set 接口的主要实现类；线程不安全的；可以存储 null 值；
                    ...
                    |---- LinkedHashSet：作为 HashSet 的子类；遍历其内部数据时，可以按照其添加的顺序遍历
       ...
       1. Set 接口中没有额外定义新的方法，使用的都是 Collection 中声明过的方法

       2. 要求：向 Set（主要指 HashSet、LinkedHashSet） 中添加的数据，其所在的类一定要重写 hashCode() 和 equals()

          要求：重写的 hashCode() 和 equals() 尽可能保持一致：相等的对象必须具有相等的散列码
...
public class SetTest {
    ...
    /*
        一、Set：存储无序的，不可重复的数据
            ...
            1. 无序性：不等于随机性。存储的数据在底层数组中并非按照数组索引的顺序添，而是根据数据的哈希值决定

            2. 不可重复性：保证添加的元素按照 equals() 方法判断时，不能返回 true。即：相同的元素只能添加一个

        二、添加元素的过程：以 HashSet 为例

            我们向 HashSet 中添加元素 a，首先调用元素 a 所在类的 hashCode() 方法，计算元素 a 的哈希值，

            此哈希值接着通过某种算法计算出在 HashSet 底层数组中的存放位置（即为：索引位置），

            判断数组此位置上是否已经有元素：

                如果此位置上没有其他元素，则元素 a 添加成功；        ---> 情况1

                如果此位置上有其他元素 b（或以链表形式存在的多个元素），首先比较元素 a 与元素 b 的 hash 值：
            ...
            对于添加成功的情况2 和 情况3 而言：元素a 与已经存在指定索引位置上的数据以链表的方式存储
    ...
    */
    ...
    // LinkHashSet 的使用：LinkHashSet 作为 HashSet 的子类，在添加数据的同时，每个数据还维护了两个引用，记录此数据前一个数据和后一个数据
